...
username = "chilloxop" # Replace this with your username
minerId = "ruby" # Custom miner identifier
    ...
    for result in 0..100 * Integer(difficulty) + 1 do 
        ...
        if sha1 == String(job[1])
            ...
            if SHAREFEED == "GOOD"
                ...
                break
            if SHAREFEED == "BLOCK"
                puts(" cpu ".colorize(:color => :white, :background => :yellow) + " Block Found".colorize(:color => :blue) + " share #" + String(sharecount) + ", speed: " + String(Integer(hashrate / 1000)) + "kH/s @ diff " + String(difficulty))
                break
            if SHAREFEED == "INVU"
